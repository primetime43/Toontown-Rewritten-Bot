...
namespace ToonTown_Rewritten_Bot
{
    public partial class MainForm
    {
        ...
        private void InitializeFishingLayout()
        {
            ...
            customFishingStatusLabel = CreateHintLabel("Ready", "customFishingStatusLabel");
            customFishingShortcutsLabel = CreateHintLabel("", "customFishingShortcutsLabel");
        ...
        private void BuildFishingTab(TabPage tab, bool custom)
        {
            ...
            var setup = CreateSettingsSection(custom ? "Route & session" : "Location & session", location,
                CreateControlRow(castsLabel, casts), CreateControlRow(sellsLabel, sells));
            left.Controls.Add(setup);
            if (custom)
            {
                createCustomFishingActionsBtn.Text = "Edit route";
                wizardCustomFishingBtn.Text = "New route";
                StyleActionButton(createCustomFishingActionsBtn);
                StyleActionButton(wizardCustomFishingBtn);
                ((TableLayoutPanel)setup).Controls.Add(CreateControlRow(createCustomFishingActionsBtn, wizardCustomFishingBtn));
                var help = CreateHintLabel("Record the walk to the fisherman and back.\nStart fishing from the same dock.");
            ...
            else
            {
                ...
                fishingLocationDescLabel.ForeColor = UiColors.MutedText;
            ...
            StyleActionButton(scan);
            StyleActionButton(colors);
            left.Controls.Add(CreateSettingsSection("Calibration", CreateControlRow(scan, colors)));
            ...
            var waitRow = CreateControlRow(CreateHintLabel("Wait limit:"), waitNumber, CreateHintLabel("sec"));
            if (custom) customShadowWaitRow = waitRow; else normalShadowWaitRow = waitRow;
            right.Controls.Add(CreateSettingsSection("Before casting", autoDetect, wait, waitRow));

            var timeout = custom ? customNumericUpDownBiteTimeout : numericUpDownBiteTimeout;
            right.Controls.Add(CreateSettingsSection("After casting", CreateControlRow(CreateHintLabel("Catch timeout:"), timeout, CreateHintLabel("sec"))));
            right.Controls.Add(CreateSettingsSection("Display & input",
            ...
            footer.Controls.Add(CreateControlRow(start, stop, status), 0, 0);
